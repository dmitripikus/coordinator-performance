...
def load(arch):
    by_size = {}
    by_size_epoch = {}
    dirs = glob.glob(f"{BASE}/{arch}/inference-perf_*_random_*_20_isl_osl_*")
    dirs = [d for d in dirs if "old" not in d.lower() and "save" not in d.lower()]
    for d in dirs:
        m = re.search(r"random_(\d+)_20_isl_osl", d)
        size = int(m.group(1))
...
plot_distribution(
    ...
    "Time to first token vs prompt size (p10-median-p90)\nprefill=gc37d06, decode=gf2a19e, matched on both sides",
    "ttft_distribution.png",
)
plot_distribution(
    ["request_latency"], 1000, "request latency (ms)",
    "Request latency vs prompt size (p10-median-p90)\nprefill=gc37d06, decode=gf2a19e, matched on both sides",
    "request_latency_distribution.png",
)
plot_distribution(
    ["inter_token_latency"], 1000, "inter-token latency (ms)",
    "Inter-token latency vs prompt size (p10-median-p90)\nprefill=gc37d06, decode=gf2a19e, matched on both sides",
